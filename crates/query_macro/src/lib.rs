...
fn fields() {
    ...
    let QueryMacro{query, struct_type, arguments} = m.unwrap();
...
fn predicates() {
    ...
    assert_eq!(f.unwrap().to_string(),"toql :: query :: Query :: < User > :: new ( ) \
    . and ( < User as toql :: query_fields :: QueryFields > :: fields ( ) . r#pred ( ) . are ( & [ ] ) ) \
    . and ( < User as toql :: query_fields :: QueryFields > :: fields ( ) . r#pred ( ) . are ( & [ \"A\" , 5u64 ] ) ) \
    . and ( < User as toql :: query_fields :: QueryFields > :: fields ( ) . r#level1 ( ) . r#pred ( ) . are ( & [ ] ) )");
}
...
fn order_and_hiding() {
    ...
    . and ( < User as toql :: query_fields :: QueryFields > :: fields ( ) . r#prop ( ) . desc ( 1u8 ) . hidden ( ) ) \
    . and ( < User as toql :: query_fields :: QueryFields > :: fields ( ) . r#prop ( ) . asc ( 1u8 ) . hidden ( ) )");
 }
#[test]
fn arguments() {
    use query_macro::QueryMacro;
